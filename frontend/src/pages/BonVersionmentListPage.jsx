          ...
          <table className="w-full table-fixed text-xs sm:text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="hidden px-2 py-3 text-left text-sm font-semibold text-gray-900 sm:table-cell sm:px-6">Émetteur</th>
            ...
            <tbody>
              {bons.map((b) => (
                <tr key={b.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="hidden px-2 py-4 text-sm text-gray-700 sm:table-cell sm:px-6">{[b.beneficiaire_prenom, b.beneficiaire_nom].filter(Boolean).join(' ') || 'Client'}</td>
                  <td className="px-2 py-4 text-sm font-semibold text-gray-900 sm:px-6">{Number(b.total_global || b.montant || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} DA</td>
                  <td className="px-2 py-4 text-sm text-gray-700 sm:px-6">{b.date_versement ? new Date(b.date_versement).toLocaleDateString('fr-FR') : '-'}</td>
